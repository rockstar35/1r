...
export default function SailingModel({ story, preview }) {
  ...
  return (
    <article>
      ...
      {preview && <ButtonPreview slug={data.slug} />}
      ...
      >
        {console.log(data.content.interior)}
      ...
      <div style={{ marginTop: '110px' }}>
        {console.log(data.content.extra_feature_content)}
...
export async function getStaticPaths() {
  const { data } = await Storyblok.get('cdn/links/', {
    starts_with: 'yachts/sailing/models/',
  })
...
export async function getStaticProps({ params, preview = false }) {
  ...
  const { data } = await Storyblok.get(
    `cdn/stories/yachts/sailing/models/${slug}`,
    sbParams
  )
